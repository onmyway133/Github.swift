...
public class Repository: Object {
  ...
  public required init(_ map: JSONDictionary) {
    ...
    self.ownerAvatarURL <- map.dictionary("owner")?.transform("avatar_url", transformer: NSURL.init(string: ))
    ...
    self.HTTPSURL <- map.transform("clone_url", transformer: NSURL.init(string: ))
    self.SSHURLString <- map.property("ssh_url")
    self.gitURL <- map.transform("git_url", transformer: NSURL.init(string: ))
